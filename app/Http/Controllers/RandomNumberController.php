...
class RandomNumberController extends Controller
{
    ...
    {
        Log::info("Cron job dijalankan pada waktu: " . Carbon::now()->format('H:i'));
        Log::info("Fungsi generateRandomNumbers dijalankan pada: " . Carbon::now());

        $variables = [];
        $threeDigitVariables = [];
        $twoDigitVariables = [];
        $singleDigitVariables = [];
        $doubleDigitVariables = [];
        $nmhewan = [];
        $fthewan = [];

        // Membuat 14 angka acak (4 digit) untuk setiap variabel
        for ($j = 1; $j <= 14; $j++) {
            $randomNumbers = '';
            for ($i = 0; $i < 4; $i++) {
                $randomNumbers .= rand(0, 9);
            }
            $variables['acak' . $j] = $randomNumbers;

            // Ambil 3 dan 2 angka terakhir
            $lastThreeDigits = substr($randomNumbers, -3);
            $lastTwoDigits = substr($randomNumbers, -2);

            // Dapatkan kombinasi unik
            $threeDigitVariables['3d' . $j] = $this->getUniqueCombinations($lastThreeDigits, 6);
            $twoDigitVariables['2d' . $j] = $this->getUniqueCombinations($lastTwoDigits, 2);

           // Tambahkan angka acak untuk $satu dan $dua
        $singleDigitVariables['satu' . $j] = rand(0, 9) . '/' . rand(0, 9); // Format "x/y"
        
        // Format "xx/yy" untuk dua angka
        $doubleDigitVariables['dua' . $j] = str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT) . '/' . str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT);

            
            // Seleksi berdasarkan dua angka terakhir
            $this->selectAnimal($lastTwoDigits, $nmhewan, $fthewan, $j);
        }

        // Menyimpan data ke file JSON
        $dataToSave = [
            'variables' => $variables,
            '3d_variables' => $threeDigitVariables,
            '2d_variables' => $twoDigitVariables,
            'single_digit_variables' => $singleDigitVariables,
            'double_digit_variables' => $doubleDigitVariables,
            'nmhewan' => $nmhewan,
            'fthewan' => $fthewan,
        ];
        
        // Simpan data ke file JSON
        file_put_contents(storage_path('random_numbers.json'), json_encode($dataToSave));
        Log::info("Data telah disimpan ke file JSON.");
        Log::info('Task Scheduler 1 - Nilai acak1 diupdate: ' . $acak1);
    }
    public function showRandomNumbers()
    {
        ...
        if (File::exists(storage_path('random_numbers.json'))) {
            $jsonContent = File::get(storage_path('random_numbers.json'));
            
            if (!empty($jsonContent)) {
                $decodedData = json_decode($jsonContent, true);
                if (is_array($decodedData)) {
                    $data = array_merge($data, $decodedData); // Gabungkan dengan default jika ada yang ada
                }
            }
        }
    
        return view('prediksi', $data); // Kirim data ke Blade
    }
