...
class GenerateAcak12 extends Command
{
    protected $signature = 'random:acak12';
    protected $description = 'Generate random number for acak12';
    ...
    public function handle()
    {
        ...
        $data['variables']['acak12'] = $acak12;

        // Ambil 3 dan 2 angka terakhir
        $lastThreeDigits = substr($acak12, -3);
        $lastTwoDigits = substr($acak12, -2);

        // Dapatkan kombinasi unik
        $data['3d_variables']['3d12'] = $this->getUniqueCombinations($lastThreeDigits, 6);
        $data['2d_variables']['2d12'] = $this->getUniqueCombinations($lastTwoDigits, 2);

        // Tambahkan angka acak untuk $satu dan $dua dengan format yang diinginkan
        $data['single_digit_variables']['satu12'] = rand(0, 9) . '/' . rand(0, 9); // Format "x/y"
        $data['double_digit_variables']['dua12'] = str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT) . '/' . str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT); // Format "xx/yy"
                
        // Seleksi berdasarkan dua angka terakhir
        $this->selectAnimal($lastTwoDigits, $data['nmhewan'], $data['fthewan'], 12);
