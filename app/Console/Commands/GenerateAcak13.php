...
class GenerateAcak13 extends Command
{ protected $signature = 'random:acak13';
    protected $description = 'Generate random number for acak13';
    ...
    public function handle()
    {
        ...
        $data['variables']['acak13'] = $acak13;

        // Ambil 3 dan 2 angka terakhir
        $lastThreeDigits = substr($acak13, -3);
        $lastTwoDigits = substr($acak13, -2);

        // Dapatkan kombinasi unik
        $data['3d_variables']['3d13'] = $this->getUniqueCombinations($lastThreeDigits, 6);
        $data['2d_variables']['2d13'] = $this->getUniqueCombinations($lastTwoDigits, 2);

        // Tambahkan angka acak untuk $satu dan $dua dengan format yang diinginkan
        $data['single_digit_variables']['satu13'] = rand(0, 9) . '/' . rand(0, 9); // Format "x/y"
        $data['double_digit_variables']['dua13'] = str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT) . '/' . str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT); // Format "xx/yy"
                
        // Seleksi berdasarkan dua angka terakhir
        $this->selectAnimal($lastTwoDigits, $data['nmhewan'], $data['fthewan'], 13);
