...
class GenerateAcak10 extends Command
{
    protected $signature = 'random:acak10';
    protected $description = 'Generate random number for acak10';
    ...
    public function handle()
    {
        ...
        $data['variables']['acak10'] = $acak10;

        // Ambil 3 dan 2 angka terakhir
        $lastThreeDigits = substr($acak10, -3);
        $lastTwoDigits = substr($acak10, -2);

        // Dapatkan kombinasi unik
        $data['3d_variables']['3d10'] = $this->getUniqueCombinations($lastThreeDigits, 6);
        $data['2d_variables']['2d10'] = $this->getUniqueCombinations($lastTwoDigits, 2);

        // Tambahkan angka acak untuk $satu dan $dua dengan format yang diinginkan
        $data['single_digit_variables']['satu10'] = rand(0, 9) . '/' . rand(0, 9); // Format "x/y"
        $data['double_digit_variables']['dua10'] = str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT) . '/' . str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT); // Format "xx/yy"
                
        // Seleksi berdasarkan dua angka terakhir
        $this->selectAnimal($lastTwoDigits, $data['nmhewan'], $data['fthewan'], 10);
