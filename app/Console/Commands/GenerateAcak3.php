...
class GenerateAcak3 extends Command
{
    protected $signature = 'random:acak3';
    protected $description = 'Generate random number for acak3';

    public function handle()
    {
        Log::info("GenerateAcak1 dijalankan pada waktu: " . Carbon::now()->format('H:i'));

        $jsonPath = storage_path('random_numbers.json');
        $data = File::exists($jsonPath) ? json_decode(File::get($jsonPath), true) : [];

        // Generate nilai acak untuk acak3
        $acak3 = '';
        for ($i = 0; $i < 4; $i++) {
            $acak3 .= rand(0, 9);
        }
        $data['variables']['acak3'] = $acak3;

        // Ambil 3 dan 2 angka terakhir
        $lastThreeDigits = substr($acak3, -3);
        $lastTwoDigits = substr($acak3, -2);

        // Dapatkan kombinasi unik
        $data['3d_variables']['3d3'] = $this->getUniqueCombinations($lastThreeDigits, 6);
        $data['2d_variables']['2d3'] = $this->getUniqueCombinations($lastTwoDigits, 2);

        // Tambahkan angka acak untuk $satu dan $dua dengan format yang diinginkan
        $data['single_digit_variables']['satu3'] = rand(0, 9) . '/' . rand(0, 9); // Format "x/y"
        $data['double_digit_variables']['dua3'] = str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT) . '/' . str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT); // Format "xx/yy"
                
        // Seleksi berdasarkan dua angka terakhir
        $this->selectAnimal($lastTwoDigits, $data['nmhewan'], $data['fthewan'], 3);
