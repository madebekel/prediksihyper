...
class GenerateAcak6 extends Command
{
    protected $signature = 'random:acak6';
    protected $description = 'Generate random number for acak6';
    ...
    public function handle()
    {
        ...
        $data['variables']['acak6'] = $acak6;

        // Ambil 3 dan 2 angka terakhir
        $lastThreeDigits = substr($acak6, -3);
        $lastTwoDigits = substr($acak6, -2);

        // Dapatkan kombinasi unik
        $data['3d_variables']['3d6'] = $this->getUniqueCombinations($lastThreeDigits, 6);
        $data['2d_variables']['2d6'] = $this->getUniqueCombinations($lastTwoDigits, 2);

        // Tambahkan angka acak untuk $satu dan $dua dengan format yang diinginkan
        $data['single_digit_variables']['satu6'] = rand(0, 9) . '/' . rand(0, 9); // Format "x/y"
        $data['double_digit_variables']['dua6'] = str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT) . '/' . str_pad(rand(0, 99), 2, '0', STR_PAD_LEFT); // Format "xx/yy"
                
        // Seleksi berdasarkan dua angka terakhir
        $this->selectAnimal($lastTwoDigits, $data['nmhewan'], $data['fthewan'], 6);
